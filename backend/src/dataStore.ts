...
import { resolveBodsConfig, type BodsConfig } from './bodsConfig.js';
...
function chooseRepresentativeTrip(routeId: string, trips: RawTripRow[], stopTimesByTrip: Map<string, RawStopTimeRow[]>): RawTripRow | undefined {
...
function buildStopsForRoute(routeId: string, trip: RawTripRow, stopTimesByTrip: Map<string, RawStopTimeRow[]>, stopsById: Map<string, RawStopRow>): Stop[] {
  const stopTimes = stopTimesByTrip.get(trip.trip_id);
  if (!stopTimes) return [];
...
function buildRouteShape(shapeId: string | undefined, shapesById: Map<string, Array<{ sequence: number; coord: [number, number] }>>): Array<[number, number]> {
  if (!shapeId) return [];
